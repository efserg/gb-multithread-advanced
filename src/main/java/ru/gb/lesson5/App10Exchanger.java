...
public class App10Exchanger {
    // Exchanger (обменник) может понадобиться, для того, чтобы
    // обменяться данными между двумя потоками в определенной точки
    // работы обоих потоков. Exchanger — обобщенный класс, он
    // параметризируется типом объекта для передачи.

    // Обменник является точкой синхронизации пары потоков: поток,
    // вызывающий у обменника метод exchange() блокируется и ждет другой
    // поток. Когда другой поток вызовет тот же метод, произойдет обмен
    // объектами: каждая из них получит аргумент другой в методе
    // exchange(). Стоит отметить, что обменник поддерживает
    // передачу null значения. Это дает возможность использовать
    // его для передачи объекта
    // в одну сторону, или, просто как точку синхронизации двух потоков.

    // Пример (https://habr.com/ru/post/277669/)
    // Рассмотрим следующий пример. Есть два грузовика: один едет из
    // пункта A в пункт D, другой из пункта B в пункт С. Дороги AD и BC
    // пересекаются в пункте E. Из пунктов A и B нужно доставить посылки
    // в пункты C и D. Для этого грузовики в пункте E должны
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        String[] p1 = new String[]{"{A->D}", "{A->C}"};
        // Формируем груз для 2-го грузовика
        String[] p2 = new String[]{"{B->C}", "{B->D}"};
